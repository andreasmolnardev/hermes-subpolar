...
_IMMUTABLE_JOB_FIELDS = frozenset({"id", "subpolar_owner", "subpolar_task_id"})
...
def create_job(
    ...
    subpolar_owner: Optional[str] = None,
    subpolar_task_id: Optional[str] = None,
) -> Dict[str, Any]:
    """
    ...
    normalized_subpolar_owner = _normalize_job_optional_text(subpolar_owner)
    normalized_subpolar_task_id = _normalize_job_optional_text(subpolar_task_id)
    if (normalized_subpolar_owner is None) != (normalized_subpolar_task_id is None):
        raise ValueError(
            "subpolar_owner and subpolar_task_id must be provided together"
        )
    ...
    if normalized_subpolar_owner is not None:
        job["subpolar_owner"] = normalized_subpolar_owner
        job["subpolar_task_id"] = normalized_subpolar_task_id
...
def _completed_oneshot_retention_days() -> float:
    ...
    try:
        ...
        return float(
            cron_cfg.get("completed_retention_days", COMPLETED_ONESHOT_RETENTION_DAYS)
        )
    except Exception:
