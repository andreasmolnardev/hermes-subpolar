...
import { createEmptyWorkspace, createNativeGitTools, gitDiff, gitStatus, validateExistingWorkspace, validateRuntimeWorkspace } from "../src/git.ts";
...
test("project workspaces stay under the configured root and Git status/diff expose agent changes", async () => {
  ...
  try {
    ...
    await assert.rejects(() => validateRuntimeWorkspace(root, workspace), /outside configured roots/);
