...
/** A capability identifier is deliberately opaque: native, MCP, and OpenAPI
 * capabilities can all use their stable resolver identifier here. */
...
export class SQLiteIdentityRepository {
  ...
  constructor(path: string) {
    ...
    try { this.db.exec("ALTER TABLE agents ADD COLUMN description TEXT NOT NULL DEFAULT ''"); } catch { /* migrated */ }
    try { this.db.exec("ALTER TABLE agents ADD COLUMN icon TEXT"); } catch { /* migrated */ }
  }
  ...
  private agent(row: Row): AgentRecord {
    ...
    return {
      ...
      description: row.description ?? "", ...(row.icon === null || row.icon === undefined ? {} : { icon: row.icon }), instructions: row.instructions ?? "",
  ...
  updateAgent(userId: string, agentId: string, input: AgentConfigurationInput): AgentRecord {
    ...
    if (!name || name.length > 128 || instructions.length > 100_000 || description.length > 10_000 || (input.icon?.length ?? 0) > 2048) throw new Error("agent is invalid");
    this.db.run("BEGIN IMMEDIATE");
    try {
      this.db.run("UPDATE agents SET name = ?, description = ?, icon = ?, instructions = ? WHERE id = ?", [name, description, input.icon === undefined ? existing.icon ?? null : input.icon || null, instructions, agentId]);
  ...
  deleteAgent(userId: string, agentId: string): void { if (this.db.run("DELETE FROM agents WHERE id = ? AND owner_id = ?", [agentId, userId]).changes !== 1) throw new OwnershipError("Agent is not owned by the authenticated user"); }

  private validateCapabilities(items: readonly AgentCapabilityAssignment[]): void { if (new Set(items.map(item => item.capabilityId)).size !== items.length || items.some(item => !item.capabilityId || item.capabilityId.length > 512 || typeof item.enabled !== "boolean")) throw new Error("agent capabilities are invalid"); }
