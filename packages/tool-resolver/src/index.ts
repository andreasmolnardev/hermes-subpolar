...
/**
 * Central authorization entrypoint for agent sessions.  Callers only supply
 * server-owned definitions and persisted assignments; client tool state is not
 * accepted here.  An unassigned capability is unavailable by default.
 */
export function resolveAgentToolDescriptors(
  ...
  for (const definition of definitions) {
    const capabilityId = definition.name;
    if (!enabled.has(capabilityId)) {
      overrides.push({ toolName: definition.name, policy: "deny" });
      continue;
    }
    let policy: ToolPolicy = policies.get(capabilityId) ?? definition.policy ?? "deny";
    // A session mode can tighten policy, but cannot defeat an explicit deny.
    if (context.sessionMode === "read-only" && isMutatingCapability(capabilityId, definition)) policy = "deny";
    else if (context.sessionMode === "ask" && policy === "allow" && isMutatingCapability(capabilityId, definition)) policy = "ask";
