...
function ProjectPrompt({
  ...
  return (
    <div className="mb-3 flex items-center justify-center gap-2 px-2 text-center text-sm text-[var(--color-muted-foreground,var(--midground-base))]">
      ...
      <Select value={project?.id ?? 'none'} onValueChange={value => setProject(value === 'none' ? '' : value)} disabled={disabled}>
        {project === undefined ? (
          <SelectTrigger
...
const MODEL_DEFAULT_FIELDS: readonly { readonly key: keyof SubpolarModelDefaults; readonly label: string; readonly description: string }[] = [
...
function ModelSettingsTabs({ providers }: { providers: boolean }) {
  return (
    <nav className="mt-6 flex gap-1 border-b border-[color-mix(in_srgb,var(--midground-base)_18%,transparent)]" aria-label="Model settings tabs">
      <NavLink to="/settings/agent/models" end className={({ isActive }) => `px-3 py-2 text-sm ${isActive && !providers ? 'border-b-2 border-[var(--color-primary,var(--midground-base))] text-[var(--foreground-base)]' : 'setup-help'}`}>
        Models
      </NavLink>
      <NavLink to="/settings/agent/models/providers" className={`px-3 py-2 text-sm ${providers ? 'border-b-2 border-[var(--color-primary,var(--midground-base))] text-[var(--foreground-base)]' : 'setup-help'}`}>
        Providers
      </NavLink>
    </nav>
  )
...
function ModelsSettings() {
  ...
  return (
    <>
      ...
      {defaults === null ? <p className="setup-help mt-6">Loading model settings...</p> : (
        <div className="mt-6 space-y-3">
          {MODEL_DEFAULT_FIELDS.map(field => (
            <label key={field.key} className="setup-option block rounded-xl p-4">
              ...
              <select value={defaults[field.key]} onChange={event => void selectModel(field.key, event.target.value)} className="setup-input mt-3 w-full" disabled={options.length === 0}>
                {options.length === 0 ? <option value={defaults[field.key]}>Configure a provider first</option> : options.map(model => <option key={model} value={model}>{model}</option>)}
              </select>
            </label>
          ))}
        </div>
      ...
      {error !== null ? <p role="alert" className="setup-error mt-4">{error}</p> : null}
    </>
  )
}
...
function ProvidersSettings() {
  ...
  const modelsFor = (provider: ModelProviderDefinition) => modelProviders.find(item => item.id === provider.id)?.models.map(model => model.label) ?? provider.fallbackModels ?? []

  function ProviderList({ title, items, configured: isConfigured }: { title: string; items: readonly ModelProviderDefinition[]; configured: boolean }) {
    return (
      <section className="mt-6">
        ...
        <div className="space-y-2">
          {items.length === 0 ? <p className="setup-option rounded-xl p-4 text-sm setup-help">{isConfigured ? 'No providers configured.' : 'All providers are configured.'}</p> : items.map(provider => (
            <article key={provider.id} className="setup-option rounded-xl p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  ...
                  {isConfigured ? <p className="mt-2 text-xs text-[var(--foreground-base)]">Models: {modelsFor(provider).join(', ') || 'No models discovered'}</p> : null}
                </div>
                <Link to={`/settings/agent/models/providers/configure?provider=${encodeURIComponent(provider.id)}`} className="setup-primary shrink-0 text-center text-sm">
  ...
  return (
    <>
      ...
      {error !== null ? <p role="alert" className="setup-error mt-6">{error}</p> : null}
      <ProviderList title="Configured providers" items={configured} configured />
      <ProviderList title="Available providers" items={available} configured={false} />
    </>
  )
}
...
function SettingsPage({ user, onLogout }: { user: SubpolarUser; onLogout: () => void }) {
  const location = useLocation()
  const { theme, baseThemeName, themeMode, availableThemes, setTheme, setThemeMode, createCustomTheme, fontId, fontChoices, setFont } = useTheme()
  ...
  function submitCustomTheme(event: FormEvent) {
    ...
    createCustomTheme({ label: customThemeLabel, background: customBackground, foreground: customForeground, accent: customAccent })
    setCustomThemeLabel('')
  }
        ...
        <aside className="flex min-h-screen min-w-0 flex-col" aria-label="Settings navigation">
          ...
          <nav className="mt-4 flex flex-col gap-1" aria-label="Settings scope">
            {(['user', 'agent'] as const).map(item => {
              ...
              return (
                <NavLink key={item} to={`/settings/${item}/${SETTINGS_SECTIONS[item][0].id}`} className={`setup-step w-full capitalize ${scope === item ? 'setup-step-active shadow-none' : ''}`}>
                  <Icon size={16} />
                  {item}
                </NavLink>
              )
          ...
          <button type="button" className="setup-step w-full justify-start md:hidden" aria-expanded={settingsSectionsOpen} onClick={() => setSettingsSectionsOpen(open => !open)}>
            <ChevronDown size={16} className={`transition-transform ${settingsSectionsOpen ? 'rotate-180' : ''}`} />
            <section.icon size={16} />
            {section.label}
          </button>
          <nav className={`${settingsSectionsOpen ? 'flex' : 'hidden'} min-h-0 flex-col gap-1 md:flex md:flex-1 md:overflow-y-auto`} aria-label={`${scope} settings sections`}>
            {SETTINGS_SECTIONS[scope].map(item => (
              <NavLink key={item.id} to={`/settings/${scope}/${item.id}`} onClick={() => setSettingsSectionsOpen(false)} className={({ isActive }) => `setup-step w-full ${isActive ? 'setup-step-active shadow-none' : 'setup-help hover:text-[var(--foreground-base)]'}`}>
                <item.icon size={16} />
                {item.label}
              </NavLink>
            ))}
        ...
        <section className="min-w-0">
          {scope === 'user' && section.id === 'appearance' ? (
            <>
              ...
              <label className="setup-label mt-6 block">Theme
                <select value={themeMode} onChange={event => setThemeMode(event.target.value as ThemeMode)} className="setup-input mt-2 w-full">
              ...
              <div className="mt-6">
                ...
                <div className="mt-2 grid gap-3 sm:grid-cols-2">
                  {availableThemes.filter(item => !item.name.startsWith('high-contrast-')).map(item => (
                    <button key={item.name} type="button" onClick={() => setTheme(item.name)} aria-pressed={baseThemeName === item.name} className={`setup-option rounded-xl p-4 text-left ${baseThemeName === item.name ? 'setup-theme-selected' : ''}`}>
                      <span className="block font-medium">{item.label}</span>
                      <span className="mt-3 flex gap-1.5" aria-label={`${item.label} colors`}>
                        {(item.definition?.swatchColors ?? [item.definition?.palette.background.hex, item.definition?.palette.midground.hex, item.definition?.colorOverrides?.primary ?? item.definition?.palette.warmGlow]).filter((color): color is string => color !== undefined).map((color, index) => (
                          <span key={`${color}-${index}`} className="h-6 flex-1 rounded-sm border border-black/10" style={{ backgroundColor: color }} />
                        ))}
                      </span>
                    </button>
                  ))}
              ...
              <form onSubmit={submitCustomTheme} className="setup-option mt-6 rounded-xl p-4">
                ...
                <label className="setup-label mt-4 block">Name
                  <input value={customThemeLabel} onChange={event => setCustomThemeLabel(event.target.value)} className="setup-input mt-2 w-full" placeholder="My theme" />
                </label>
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  {([['Background', customBackground, setCustomBackground], ['Foreground', customForeground, setCustomForeground], ['Accent', customAccent, setCustomAccent]] as const).map(([label, value, setValue]) => (
                    <label key={label} className="setup-label">{label}
                      <input type="color" value={value} onChange={event => setValue(event.target.value)} className="mt-2 h-10 w-full cursor-pointer rounded border border-white/10 bg-transparent p-1" />
                    </label>
                  ))}
                </div>
                <button type="submit" disabled={!customThemeLabel.trim()} className="setup-primary mt-4 disabled:opacity-40">Create theme</button>
              </form>
            ...
            providersTab ? <ProvidersSettings /> : <ModelsSettings />
          ) : (
            <>
  ...
  return (
    <Routes>
      ...
      <Route path="/apps" element={<Workspace user={user} onLogout={onLogout} />} />
       ...
       <Route path="/settings/agent/models/providers/configure" element={<ProviderSetupScreen editing onComplete={() => routerNavigate('/settings/agent/models/providers')} />} />
