...
import { isSubpolarTerminalEvent, subpolarEventType, SubpolarWebSocketClient, type SubpolarSocketEvent } from "@/lib/subpolar-client";
...
function Chat({
  ...
  setPermissionMode: (value: "full" | "ask" | "read-only") => void;
  streaming: boolean;
  onSend: () => void;
  onCancel: () => void;
}) {
...
function Workspace({ user, onLogout }: { user: SubpolarUser; onLogout: () => void }) {
  ...
  const [permissionMode, setPermissionMode] = useState<"full" | "ask" | "read-only">("ask");
  ...
  function handleEvent(requestId: string, event: SubpolarSocketEvent): void {
    ...
    if (type === "approval.request") {
      const callId = typeof eventRecord(event.event)?.payload === "object" && eventRecord(eventRecord(event.event)?.payload)?.call_id;
      if (typeof callId === "string") clientRef.current?.respondToApproval(callId, window.confirm("Allow this agent tool invocation?") ? "allow" : "deny");
    }
    ...
    if (isSubpolarTerminalEvent(event)) {
      setStreaming(false);
      requestRef.current = null;
      if (type === "message.complete") void refreshSessions();
      if (type === "error") setError("The response could not be completed.");
  ...
  async function send(): Promise<void> {
    ...
    const client = new SubpolarWebSocketClient({ onEvent: event => handleEvent(requestId, event), onClose: (reconnecting, reason) => { if (!reconnecting && requestRef.current === requestId) { setStreaming(false); requestRef.current = null; if (reason !== "cancelled") setError("Connection closed while the response was streaming."); } } });
    clientRef.current = client;
    try { const outgoing: readonly SubpolarMessage[] = [...messages, { role: "user", content: text }]; await client.start({ requestId, sessionId, model, permissionMode, messages: outgoing.filter(message => message.role !== "tool").map(message => ({ role: message.role as "system" | "user" | "assistant", content: messageText(message) })), ...(selectedProject ? { projectId: selectedProject } : {}), ...(selectedAgent ? { agentId: selectedAgent } : {}) }); } catch { setStreaming(false); requestRef.current = null; setError("Could not connect to the agent."); client.close(); }
  }
