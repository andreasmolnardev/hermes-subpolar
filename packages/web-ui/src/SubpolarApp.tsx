...
function messageText(message: SubpolarMessage) {
...
function Workspace({ user, onLogout }: { user: SubpolarUser; onLogout: () => void }) {
  ...
  const [view, setView] = useState<View>('chat'),
    ...
    [messages, setMessages] = useState<readonly SubpolarMessage[]>([]),
  ...
  async function send() {
    ...
    try {
      await client.start({
        ...
        sessionId,
        model,
        messages: [...messages, { role: 'user', content: text } as SubpolarMessage].map(m => ({
