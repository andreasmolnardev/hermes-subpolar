...
Auth for the [web dashboard](/user-guide/features/web-dashboard). Per the
secrets-only convention, credentials belong in `~/.hermes/.env`; the OAuth
`client_id` is better set under `dashboard.oauth` in `config.yaml` (env wins
when set).

Three dashboard-auth providers ship in the box. For any internet-facing
dashboard, the recommended provider is **OAuth (Nous Portal)**. The bundled
**username/password** provider (`HERMES_DASHBOARD_BASIC_AUTH_*`) is quickest on
a trusted LAN or behind a VPN. To authenticate against your own identity
provider, use **self-hosted OIDC** (`HERMES_DASHBOARD_OIDC_*`). A non-loopback
bind engages the auth gate. See [Web Dashboard -> Authentication](/user-guide/features/web-dashboard#authentication-gated-mode).
...
| `HERMES_DASHBOARD_BASIC_AUTH_SECRET` | HMAC key (32+ bytes, base64/hex/raw) signing self-hosted session tokens. Overrides `dashboard.basic_auth.secret`; Subpolar persists generated key material under `HERMES_HOME` when blank. |
| `HERMES_DASHBOARD_BASIC_AUTH_RESET_TOKEN` | Optional operator-injected reset secret. Enables self-hosted password reset without storing reset secret on disk. |
| `HERMES_DASHBOARD_BASIC_AUTH_ALLOW_REGISTRATION` | Enables public self-hosted signup after first-user bootstrap. Default disabled; first-user bootstrap remains available when Subpolar bootstrap mode is enabled. |
...
| `HERMES_TUI_DIR` | Path to a prebuilt `ui-tui/` directory (must contain `dist/entry.js` and populated `node_modules`). Used by distributions to skip the first-launch `npm install`. |
